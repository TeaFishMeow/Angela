...
def bake_visemes(input_wav: Path, output_json: Path, song_id: str, fps: int = 30) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bake Live2D mouth-open visemes from Angela vocal wav.")
    parser.add_argument("input_wav", type=Path)
    parser.add_argument("output_json", type=Path)
    parser.add_argument("--song", default="daoxiang")
    parser.add_argument("--fps", type=int, default=30)
    args = parser.parse_args()
    bake_visemes(args.input_wav, args.output_json, args.song, args.fps)
